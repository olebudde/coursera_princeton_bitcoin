				...
				for (String s1:args1) {
					for (String s2:args2) {
						for (String s3:args3) {
							...
							a[0] = s0;  // p_graph (.1, .2, .3),
							a[1] = s1; 	// p_malicious (.15, .30, .45)
							a[2] = s2; 	// p_txDistribution (.01, .05, .10),
							a[3] = s3;  // numRounds (10, 20)
							s.main2(a);
						}
					}
				}								
	...
	public void main2(String[] args) {
		...
		for (int i = 0; i < numNodes; i++) {
			...
			for (Result tmp:result) {
				if (tmp.transactions.equals(transactions)) {
					foundIt = true;
					tmp.cnt++;
				}
			}
		...
		for (Result r:result) {    	  
			tmp[index++] = String.format("Size: %3d nodes: %3d",r.transactions.size(),r.cnt);

		}
		Arrays.sort(tmp);
		System.out.println(arrayToString(args) + ": " + reverseArrayToString(tmp));

	}


	private static String reverseArrayToString(String s[]) {
		String retval = "";
